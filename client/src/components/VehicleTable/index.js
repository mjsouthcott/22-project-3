...
function getVehicleIconBackgroundColor (vehicle) {
  if (vehicle.repairRequests.length) {
    ...
    else if (vehicle.repairRequests.slice(-1)[0].status === "Work In Progress") return "gold"
  }
  return "green"
}
                ...
                {props.vehicles.map(vehicle => (
                  <TableRow key={vehicle._id}>
                    <TableCell component="th" scope="row">
                      <VehicleAvatar icon={vehicle.iconSrc} alt={vehicle.type} backgroundColor={getVehicleIconBackgroundColor(vehicle)} imageWidth={80}/>
                    </TableCell>
